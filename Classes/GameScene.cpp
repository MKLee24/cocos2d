...
bool GameScene::init()
{
	...
	// 创建暂停按钮,返回按钮和控制音乐按钮.
	btn3 = MenuItemImage::create("pause.png", "paly.png", CC_CALLBACK_0(GameScene::pauseGame, this));
...
void GameScene::BombAndFish(int ty) {
	...
	for (int i = 0; i < enemyList.size(); i++)
	{
		...
		if (bloodList[i] <= 0) {
			...
			if (score > 999) {
				// 保存最短时间
				int GuanNum = (GameOverScene::win_num - 1) / 2;
				//RankScene::bestTime[GuanNum] = costTime;
				if (GuanNum == 0) {
					float temTime = UserDefault::getInstance()->getFloatForKey("time1");
					if (costTime < temTime)
						UserDefault::getInstance()->setFloatForKey("time1", costTime);
				}
				else if (GuanNum == 1) {
					float temTime = UserDefault::getInstance()->getFloatForKey("time2");
					if (costTime < temTime)
						UserDefault::getInstance()->setFloatForKey("time2", costTime);
				}
				else if (GuanNum == 2) {
					float temTime = UserDefault::getInstance()->getFloatForKey("time3");
					if (costTime < temTime)
						UserDefault::getInstance()->setFloatForKey("time3", costTime);
				}
				else if (GuanNum == 3) {
					float temTime = UserDefault::getInstance()->getFloatForKey("time4");
					if (costTime < temTime)
						UserDefault::getInstance()->setFloatForKey("time4", costTime);
				}
				else if (GuanNum == 4) {
					float temTime = UserDefault::getInstance()->getFloatForKey("time5");
					if (costTime < temTime)
						UserDefault::getInstance()->setFloatForKey("time5", costTime);
				}
...
void GameScene::pauseGame()
{
	if (!isPause) {
		...
		btn3->setNormalImage(Sprite::create("play.png"));
		...
		btn3->setNormalImage(Sprite::create("pause.png"));
	}
}
...
void GameScene::soundUsing()
{
	if (!useBgm) {
		...
		btn5->setNormalImage(Sprite::create("musicoff.png"));
	}
	else {
		SimpleAudioEngine::getInstance()->stopBackgroundMusic();
		useBgm = false;

		btn5->setNormalImage(Sprite::create("musicon.png"));
	}
}