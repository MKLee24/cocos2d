...
bool HelloWorld::init()
{
	...
	// 查看最佳纪录榜
	auto bestTime = MenuItemImage::create("paihangbang.png", "paihangbang2.png",
		CC_CALLBACK_1(HelloWorld::menuRankCallback, this));
	bestTime->setPosition(Vec2(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y - 3 * startItem->getContentSize().height));
	//结束
	auto closeItem = MenuItemImage::create("exit.png", "exit2.png",
		CC_CALLBACK_1(HelloWorld::menuCloseCallback, this));
	closeItem->setPosition(Vec2(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y - 4 * startItem->getContentSize().height));
	auto menu = Menu::create(startItem, chooseGuan, chooseMan, bestTime, closeItem, NULL);
...
void HelloWorld::menuRankCallback(Ref* pSender) {
