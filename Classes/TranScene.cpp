...
int TranScene::Checkpoint = 1;

Scene* TranScene::createScene()
{
...
bool TranScene::init()
{
	...
	if (Checkpoint == 1) {
		...
		auto tr = Sprite::create("tran_1.png");
		tr->setPosition(Vec2(visibleSize.width / 2, visibleSize.height / 2));
		tr->setAnchorPoint(Vec2(0.5, 0.5));
		this->addChild(tr, 1);
...
void TranScene::menuMagic1Callback(Ref* pSender)
{
	...
	if (Checkpoint == 1) {
		...
		auto scene = FirthBossScene::createScene();
		auto gameScene = TransitionSlideInR::create(1.0f, scene);
		Director::getInstance()->replaceScene(gameScene);
	}
}
